...
if (strlen($start_date) == 8){
	// 기존에 들어있는 값이 있을 경우엔 기존의 값을 이용한다.
	//(int)
...
<section id="bo_w">
		...
		<tr>
			...
			<td>
				...
				<select onchange="javascript:resetday('from');" name="fday">
					<?=$htm_fday?>
				</select> 일<input type="text" name="wr_link1" itemname="링크 #1" value="<?=$f_date?>" class="input" size="60">
			</td>
		</tr>
		<tr>
			<th>종료일</th>
			<td>
				...
				<select onchange="javascript:resetday('to');" name="tday">
					<?=$htm_tday?>
				</select> 일<input type="text" name="wr_link2" itemname="링크 #2" value="<?=$t_date?>" class="input" size="60">
			</td>
		</tr>
        <?php if ($is_name) { ?>
        <tr>
	...
	function resetday(a_val) {
		...
		if (a_val=="from") {
			f.wr_link1.value = f.fyear.value+f.fmon.value+f.fday.value;
		}
		if (a_val=="to") {
			f.wr_link2.value = f.tyear.value+f.tmon.value+f.tday.value;
		}
